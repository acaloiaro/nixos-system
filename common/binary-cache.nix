{
  ...
  cachixTokenConf = "/etc/nix/cachix-access-tokens.conf";
in {
  options = {
  ...
  config = {
    nix = {
      ...
      extraOptions = ''
        !include ${cachixTokenConf}
      '';
    };

    system.activationScripts.nix-cachix-token = {
      deps = ["agenix"];
      text = ''
        echo "access-tokens = cachix.org=$(cat ${config.age.secrets.cachix.path})" \
          > ${cachixTokenConf}
        chmod 600 ${cachixTokenConf}
      '';
    };
  };
}
